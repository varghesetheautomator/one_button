...
class Roof_Cash_Page(Solar_Sunnova_Page):
    ...
    def get_instance(cls):
        if cls.instance is None:
            cls.instance = Roof_Cash_Page()
        return cls.instance

    def __init__(self):
        super().__init__()

    def end_to_end_roof_cash(self):
        try:

            log.logger.info(
                "Started Test for End to End Workflow of Product Roof with Cash as Finance Method")
            fetch_opportunity_err = self.fetch_opportunity(
                opportunity_id.ROOF_CASH)
            for i in range(1):
                ...
                break
            log.logger.info(
                "Completed Test for End to End Workflow of Product Roof with Cash as Finance Method")
